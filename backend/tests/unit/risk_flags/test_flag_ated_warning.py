"""Tests for ATED_WARNING risk flag.

Condition: LIMITED_COMPANY AND purchase_price > 500000  Severity: MEDIUM
"""
import dataclasses
...
class TestATEDWarning:

    def test_fires_for_ltd_co_above_500k(self) -> None:
        flags = evaluate_flags(_ctx(
            ownership_structure=OwnershipStructure.LIMITED_COMPANY,
            income_tax_band=None,
            purchase_price=Decimal("500001"),
        ))
        assert "ATED_WARNING" in _codes(flags)

    def test_does_not_fire_at_exactly_500k(self) -> None:
        flags = evaluate_flags(_ctx(
            ownership_structure=OwnershipStructure.LIMITED_COMPANY,
            income_tax_band=None,
            purchase_price=Decimal("500000"),
        ))
        assert "ATED_WARNING" not in _codes(flags)

    def test_does_not_fire_for_individual_above_500k(self) -> None:
    ...
    def test_triggered_by_field_and_value(self) -> None:
        ...
        flag = next(f for f in flags if f.code == "ATED_WARNING")
        assert flag.triggered_by_field == "purchase_price"
        assert flag.triggered_by_value == "600000.00"
