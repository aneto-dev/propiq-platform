"""Tests for LOW_NET_YIELD risk flag.

Condition: net_yield_percent < 3.0  Severity: MEDIUM
"""
import dataclasses
...
class TestLowNetYield:

    def test_fires_at_2_99(self) -> None:
        """net_yield_percent=2.99 fires. Source: TEST_STRATEGY.md 6.3."""
        flags = evaluate_flags(_ctx(net_yield_percent=Decimal("2.99")))
        assert "LOW_NET_YIELD" in _codes(flags)

    def test_does_not_fire_at_3_00(self) -> None:
        """Boundary: 3.00 must NOT fire. Source: TEST_STRATEGY.md 6.3."""
        flags = evaluate_flags(_ctx(net_yield_percent=Decimal("3.00")))
        assert "LOW_NET_YIELD" not in _codes(flags)

    def test_does_not_fire_above_threshold(self) -> None:
        assert "LOW_NET_YIELD" not in _codes(evaluate_flags(_CTX))

    def test_severity_is_medium(self) -> None:
        flags = evaluate_flags(_ctx(net_yield_percent=Decimal("2.00")))
        flag = next(f for f in flags if f.code == "LOW_NET_YIELD")
