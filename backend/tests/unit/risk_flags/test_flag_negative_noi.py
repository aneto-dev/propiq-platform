"""Tests for NEGATIVE_NOI risk flag.

Condition: net_operating_income < 0  Severity: HIGH
"""
import dataclasses
...
class TestNegativeNOI:

    def test_fires_when_negative(self) -> None:
        """net_operating_income=-0.01 fires. Source: TEST_STRATEGY.md 6.3."""
        flags = evaluate_flags(_ctx(net_operating_income=Decimal("-0.01")))
        assert "NEGATIVE_NOI" in _codes(flags)

    def test_does_not_fire_at_zero(self) -> None:
        """net_operating_income=0.00 does NOT fire. Source: TEST_STRATEGY.md 6.3."""
        flags = evaluate_flags(_ctx(net_operating_income=Decimal("0")))
        assert "NEGATIVE_NOI" not in _codes(flags)

    def test_does_not_fire_when_positive(self) -> None:
        assert "NEGATIVE_NOI" not in _codes(evaluate_flags(_CTX))

    def test_triggered_by_field(self) -> None:
        flags = evaluate_flags(_ctx(net_operating_income=Decimal("-500")))
        flag = next(f for f in flags if f.code == "NEGATIVE_NOI")
        assert flag.triggered_by_field == "net_operating_income_gbp"
