"""Tests for SECTION_24_IMPACT risk flag.

Condition: INDIVIDUAL AND income_tax_band in HIGHER/ADDITIONAL  Severity: HIGH
"""
import dataclasses
...
class TestSection24Impact:

    def test_fires_for_higher_rate_individual(self) -> None:
        flags = evaluate_flags(_ctx(ownership_structure=OwnershipStructure.INDIVIDUAL,
            income_tax_band=IncomeTaxBand.HIGHER_RATE))
        assert "SECTION_24_IMPACT" in _codes(flags)

    def test_fires_for_additional_rate_individual(self) -> None:
        flags = evaluate_flags(_ctx(ownership_structure=OwnershipStructure.INDIVIDUAL,
            income_tax_band=IncomeTaxBand.ADDITIONAL_RATE))
        assert "SECTION_24_IMPACT" in _codes(flags)

    def test_does_not_fire_for_basic_rate(self) -> None:
        assert "SECTION_24_IMPACT" not in _codes(evaluate_flags(_CTX))
    ...
    def test_does_not_fire_for_ltd_co(self) -> None:
        ...
        assert "SECTION_24_IMPACT" not in _codes(flags)

    def test_triggered_by_field_and_value(self) -> None:
        flags = evaluate_flags(_ctx(ownership_structure=OwnershipStructure.INDIVIDUAL,
            income_tax_band=IncomeTaxBand.HIGHER_RATE))
        flag = next(f for f in flags if f.code == "SECTION_24_IMPACT")
        assert flag.triggered_by_field == "income_tax_band"
        assert flag.triggered_by_value == "HIGHER_RATE"
