...
- [x] `make migrate` completes cleanly against fresh PostgreSQL
- [ ] `make seed` inserts all 5 seed records without error
- [ ] Re-running `make seed` is idempotent
- [ ] Schema integrity tests pass
- [x] PostGIS extension enabled (verified via `SELECT PostGIS_Version()`)

**Migration implementation note (recorded 2026-06-06):**
The original hand-written migrations (0001, 0002) were replaced with a single autogenerated migration on branch `clean-migration-baseline`. The new migration uses SQLAlchemy's native `sa.Enum(..., name='...')` enum handling inside `op.create_table()` instead of manual `op.execute("CREATE TYPE ...")` statements. This eliminates the `DuplicateObjectError` that occurred when two session-scoped fixtures both invoked `alembic upgrade head` against the same test database during `make test-int`. The circular FK between `deals` ↔ `snapshot_calculations` is resolved by deferring `deals.latest_snapshot_id → snapshot_calculations` via `op.create_foreign_key()` after both tables exist. The `0002_database_roles.py` migration (propiq_app / propiq_admin role grants) was removed; database role privileges will be re-added as a separate migration in Phase 8 (deployment).
